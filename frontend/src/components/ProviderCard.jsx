    ...
    <Link to={`/profile/${id}`} >
      <div className='bg-[#D9D9D9] py-4 flex flex-col justify-between  text-[#032F3C] px-3 rounded-2xl w-[250px] h-[350px] relative'>
        <div className=' flex justify-around items-center'>
          <img src={user?.services[0]?.img.asset.url} className='rounded-[50%] w-[100px] h-[100px]'/>
          <div> {user?.uname}</div>
        </div>
        <div className='text-[#85D027] text-center'>{ user?.isAvailable ? 'available' : 'unavailable' }  </div>
        <div> service: {user?.services[0]?.title} </div>
        <div>location :{cityData?.display_name }  </div>
        <div className='bg-[#85D027] w-fit py-2 px-3 text-[#000000] mx-auto rounded-xl'>Price per hour : {user?.services[0]?.price} DA </div>
